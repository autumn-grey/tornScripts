...
"use strict";
(() => {
  // src/oc-travel-guard/raccoonAngry.png
  ...
  var STATUS_RETURN_ID = "ocg-status-return";
  var STATUS_MESSAGE_ID = "ocg-status-message";
  var REPORT_CLASS = "ocg-report";
  var REPORT_COPY_CLASS = "ocg-report-copy";
  var REPORT_URL = "https://www.torn.com/messages.php#/p=compose&XID=4386333";
  var TRAVEL_COUNTRIES = [
    "Mexico",
    "Cayman Islands",
    "Canada",
    "Hawaii",
  ...
  var ocCapturedText = "";
  function rememberTooltipText(text) {
  ...
  function triggerOcTooltip(element, entering) {
    ...
    const propNames = entering ? ["onMouseEnter", "onPointerEnter", "onMouseOver", "onFocus"] : ["onMouseLeave", "onPointerLeave", "onMouseOut", "onBlur"];
    while (fiber) {
    ...
    const nativeEvents = entering ? ["pointerenter", "mouseenter", "mouseover", "focus"] : ["pointerleave", "mouseleave", "mouseout", "blur"];
  ...
  var OC_RETRY_MIN_MS = 1e3;
  var OC_RETRY_MAX_MS = 3e4;
  var ocRetryDelayMs = OC_RETRY_MIN_MS;
  var ocRetryTimer = 0;
  function reopenOcLookup() {
    const previous = ocState;
    ocState = OC_UNKNOWN;
    ocAttempts = 0;
    return previous;
  }
  function scheduleOcRetry() {
  ...
  function installOverlayReconciler() {
  ...
  var REPORT_GAP_PX = 8;
  var REPORT_WIDTH_PX = 260;
  var reportBoxes = /* @__PURE__ */ new Map();
  function copyText(text) {
    const selectAndCopy = () => new Promise((resolve, reject) => {
  ...
  function buildReportBox() {
    const box = document.createElement("div");
    box.className = `${REPORT_CLASS} ${OWN_CLASS}`;
    const intro = document.createElement("div");
    intro.innerHTML = "Hey! Congrats, you found a <i>newww</i> thing!! Can you please copy this (just click it to copy):";
    box.appendChild(intro);
    const payload = ocCapturedText === "" ? "(no tooltip text found)" : ocCapturedText;
  ...
  function findSelectedDestination() {
    for (const button of document.querySelectorAll(
      ...
      if (country !== void 0 && TRAVEL_COUNTRIES.includes(country)) {
        return country;
      }
    }
  ...
  function findDestinationCandidates() {
    const results = [];
    for (const element of document.body.querySelectorAll("*")) {
      if (element.children.length > 0) continue;
      const text = (element.textContent ?? "").trim();
  ...
  function formatDuration(ms, withSeconds) {
    ...
    const clock = withSeconds ? `${hours}:${pad(minutes)}:${pad(seconds)}` : `${hours}:${pad(minutes)}`;
    return days > 0 ? `${days}d ${clock}` : clock;
  }
  function formatClockTCT(ms) {
  ...
  function updateStatusBar() {
    ...
    if (ocState === OC_RECRUITING || ocState === OC_NONE) {
      ...
      messageEl.innerHTML = ocState === OC_RECRUITING ? "Travel to your heart's content queen x" : "Hey bae, maybe join an OC?";
      return;
    }
    if (ocStartMs === null) {
      messageEl.style.display = "none";
  ...
  async function main() {
    ...
    window.setInterval(updateStatusBar, 1e3);
